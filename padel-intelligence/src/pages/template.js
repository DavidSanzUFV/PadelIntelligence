...
const Prediction = () => {
 ...
 useEffect(() => {
  ...
  const handlePredict = () => {
    ...
    fetch("http://localhost:8000/run_prediction/", {
      method: "POST",
      ...
      .then((data) => {
        console.log("Datos obtenidos del backend (formato):", JSON.stringify(data, null, 2));
        setIsLoading(false);
        if (data.error && data.error[0]) {
            setPredictionOutput("Error: " + data.error[1]);
        } else if (data.game_probability && data.set_probability && data.match_probability) {
            // Desglosa el contenido de manera más clara
            const gameProbs = JSON.stringify(data.game_probability, null, 2);
            const setProbs = JSON.stringify(data.set_probability, null, 2);
            const matchProbs = JSON.stringify(data.match_probability, null, 2);
